...
const Deals = () => {
  ...
  const getDetails = useCallback(async type => {
    setIsLoading(true);
    ...
    if (!_.isEmpty(result?.data?.totalWalletBusiness)) {
      let temp = [...result?.data?.totalWalletBusiness]
        ...
        .sort((a, b) => {
          if (
            (a?.deal?.redeem_details?.is_complete_redeemed === undefined ||
              a?.deal?.redeem_details?.is_complete_redeemed === null) &&
            a?.is_redeemed === 0
          ) {
            return -1;
          }
          if (
            (b?.deal?.redeem_details?.is_complete_redeemed === undefined ||
              b?.deal?.redeem_details?.is_complete_redeemed === null) &&
            b?.is_redeemed === 0
          ) {
            return 1;
          }
          ...
          const aIsCompleteRedeemed =
            a?.deal?.redeem_details?.is_complete_redeemed ?? 0;
          const bIsCompleteRedeemed =
            b?.deal?.redeem_details?.is_complete_redeemed ?? 0;
  ...
  const renderItem = useCallback(
    ({item, index}) => (
      <WalletViewItems
        ...
        onRefresh={() => getDetails(filterOption)}
      />
    ),
    [],
  );
